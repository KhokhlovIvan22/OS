...
DWORD RunProcess(const wstring& commandLine) {
    ...
    BOOL ok = CreateProcessW(nullptr, cmdPtr, nullptr, nullptr, FALSE, 0, nullptr, nullptr, &si, &pi);
    assert(ok);
...
DWORD RunProcessInput(const wstring& commandLine, const string& inputData) {
    ...
    PROCESS_INFORMATION pi{};
    BOOL ok = CreateProcessW(nullptr, cmdPtr, nullptr, nullptr, TRUE, 0, nullptr, nullptr, &si, &pi);
    assert(ok);
    CloseHandle(hRead);
