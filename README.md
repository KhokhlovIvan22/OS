...
- **Receiver** — retrieves messages from the queue, processes them, and outputs results to the console.
- The design emphasizes explicit contracts between processes, clear error messaging, and robust input validation.
- End-to-end tests verify correct interaction between sender and receiver, ensuring that invalid inputs are rejected and that message passing works reliably.
...
Key files:
- **filequeue.cpp / filequeue.h** — implement the shared queue abstraction with push/pop operations, input normalization, and error handling.
- **sender.cpp** — console utility that accepts integer input, validates it, and pushes messages into the queue.
- **receiver.cpp** — console utility that retrieves messages from the queue and prints them with clear process prefixes.
